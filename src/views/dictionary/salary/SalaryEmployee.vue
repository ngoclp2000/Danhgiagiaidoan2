        ...
        <div class="content-grid">
          ...
          <div class="h2p5 sub-grid-container">
            <div class="grid-header">
              ...
              <div class="icon-zoom" @click="toggleFullscreenSubTable" v-tooltip="tooltipContent['zoom']">

              </div>
            </div>
            <BaseTable :classListGridContainer="'sub-grid'" ref="subTable" :displayFunction="false"
              :summarizeField="summarizeFieldName" :displayPaging="false" :disableCheckBox="true" :type="type"
              :ths="thsSub" />
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    async mounted() {
      ...
      // Sự kiện resize bảng
      let resizeIcon = document.querySelector('.salary-content .resize-icon');
      resizeIcon.addEventListener("mousedown",  () =>{
        resizeIcon.onmousemove = (e)=>{
          this.mouseMoveFunction(e,resizeIcon);
        }
      });
      resizeIcon.addEventListener("mouseup", function () {
        resizeIcon.onmousemove = null
      });
    ...
    methods: {
      mouseMoveFunction(event,resizeIcon){
        var resizeIconRect = resizeIcon.getBoundingClientRect();
        console.log(event,resizeIconRect.top);
      },
      /**
       * Ẩn form nhân viên
        ...
        try {
          ...
          if (res.status != MISACODE.NoContent) {
            ...
            this.$nextTick(async () => {
              ...
              this.$refs.salary.$refs.formSalaryTable.checkedList = [...newList];
              this.$refs.salary.$refs.formSalaryTable.sortDirectionList = [...newList];
       ...
       */
      async updateSubTable() {
        // Chuyển trạng thái không sẵn sàng cho bảng phụ
        this.$refs["subTable"].isReady = false;
        let periodSalaryId = this.$refs["mainTable"].selectedList[0].PeriodSalaryId;
        try {
          let params = {
      ...
      async refreshData() {
        ...
        try {
          ...
          let res = await PeriodSalaryAPI.filterData(this.$store.state.data.payload);
          ...
          this.$refs.mainTable.checkedList = [...newList];
          ...
          if (this.$refs.mainTable.$refs.all.checked) {
            this.$refs.mainTable.$refs.all.click();
          }
